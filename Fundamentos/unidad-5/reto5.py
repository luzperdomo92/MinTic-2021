...
arrival_time_motorcycle = None
session = True

current_location_coordinates = None
min_distance_selected = None
...
def arrival_time(place_coordenates):
    global arrival_time_motorcycle

    speed_moto =  19.44
    speed_bicicleta =  3.33

    arrival_time_motorcycle = place_coordenates[3] / speed_moto
    arrival_time_motorcycle = int(arrival_time_motorcycle)
    time_bicicleta = place_coordenates[3] / speed_bicicleta
    time_bicicleta = int(time_bicicleta)

    return arrival_time_motorcycle, time_bicicleta
...
def followDirection(user_coordenates, place_coordenates):
    ...
    moto, bicicleta = arrival_time(place_coordenates)
...
def searchWifi(coords_to_compare):
    ...
    try:
        ...
        if option_location == "1":
            min_distance_selected = min_distance

        elif option_location == "2":
            min_distance_selected = second_min_distance
        ...
        followDirection(coords_to_compare, min_distance_selected)
    except ValueError:
        print("Error")
        session = False
        return
...
def searchNearhWifiZone(_option):
    global session
    global coordinates
    global current_location_coordinates
    validate_coordenates = np.all(coordinates)
    ...
    if validate_coordenates:
        ...
        try:
            ...
            if current_location == "1":
                current_location_coordinates = coordinates[0].tolist()

            elif current_location == "2":
                current_location_coordinates = coordinates[1].tolist()

            elif current_location == "3":
                current_location_coordinates = coordinates[2].tolist()
            else:
                print("Error ubicación")
                session = False
                return

            searchWifi(current_location_coordinates)
        except ValueError:
            print("Error")
            session = False
            return
...
################ MENU OPTION 4 #############
def saveFileNearLocation(_option):
    global session
    global coordinates
    global arrival_time_motorcycle
    global min_distance_selected
    ...
    information = {
            'actual': [None, None],
            'zonawifi1': [None, None, None],
            'recorrido': [None, 'Moto', None]
        }


    if validate_coordenates and current_location_coordinates:

        information["actual"] = current_location_coordinates
        information["zonawifi1"] = min_distance_selected[:3]
        information["recorrido"][0] = min_distance_selected[3]
        information["recorrido"][2] = arrival_time_motorcycle


        print(information)

        try:
            confirm_file_export = input("¿Está de acuerdo con la información a exportar?\n"
                "Presione 1 para confirmar, 0 para regresar al menú principal\n")

            if confirm_file_export == "1":
                with open("information_data.txt",'w',encoding = 'utf-8') as f:
                    f.write(str(information))

                print("Exportando archivo")
                session = False

            elif confirm_file_export == "0":
                displayMenu()
...
def updateFiles(_option):
    ...
    if validate_coordenates and current_location_coordinates:
        ...
        with open("information_data.txt",'r',encoding = 'utf-8') as f:
            print(f.read())

        try:
            user_option = input("Datos de coordenadas para zonas wifi actualizados,\n"
